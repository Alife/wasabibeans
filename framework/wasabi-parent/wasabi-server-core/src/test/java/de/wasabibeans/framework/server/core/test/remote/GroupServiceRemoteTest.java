...
public class GroupServiceRemoteTest extends WasabiRemoteTest {
	...
	public void get1DisplayNameTest() throws Exception {
		String displayName = groupService().getDisplayName(group1_1);
	...
	public void get1ParentGroupTest() throws Exception {
		WasabiGroupDTO group1 = groupService().getParentGroup(group1_1);
		AssertJUnit.assertEquals(groupService().getName(group1), "group1");

		AssertJUnit.assertNull(groupService().getParentGroup(group1));
	}
	...
	public void get1SubGroupByNameTest() throws Exception {
		...
		AssertJUnit.assertEquals("group1_1_2", groupService().getName(group1_1_2));
	...
	public void get1SubGroupsTest() throws Exception {
		...
		for (WasabiGroupDTO subgroup : subgroups) {
			if (groupService().getName(subgroup).equals("group1_1_1")
					|| groupService().getName(subgroup).equals("group1_1_2")) {
				hitcount++;
			}
		}
	...
	public void get1TopLevelGroupsTest() throws Exception {
		...
		for (WasabiGroupDTO subgroup : topgroups) {
			if (groupService().getName(subgroup).equals("group1")
					|| groupService().getName(subgroup).equals(WasabiConstants.WASABI_GROUP_NAME)
					|| groupService().getName(subgroup).equals(WasabiConstants.ADMINS_GROUP_NAME)) {
				hitcount++;
			}
		}
	...
	public void get1MembersTest() throws Exception {
		...
		for (WasabiUserDTO member : members) {
			if (userService().getName(member).equals("user1") || userService().getName(member).equals("user2")) {
				hitcount++;
			}
		}
		AssertJUnit.assertEquals(2, hitcount);
	...
	public void get1MemberByNameTest() throws Exception {
		...
		AssertJUnit.assertEquals("user2", userService().getName(user2));
	...
	public void createTest() throws Exception {
		...
		AssertJUnit.assertEquals("group2", groupService().getDisplayName(group2));
		Vector<WasabiGroupDTO> topgroups = groupService().getTopLevelGroups();
		AssertJUnit.assertTrue(topgroups.contains(group2));

		WasabiGroupDTO group1_1_3 = groupService().create("group1_1_3", group1_1);
		AssertJUnit.assertEquals("group1_1_3", groupService().getDisplayName(group1_1_3));
		Vector<WasabiGroupDTO> subgroups = groupService().getSubGroups(group1_1);
	...
	public void moveTest() throws Exception {
		...
		groupService().move(group1_1_2, group1);
		...
		groupService().move(group1_1_2, null);
	...
	public void renameTest() throws Exception {
		WasabiGroupDTO group1 = groupService().getGroupByName("group1");
		try {
			groupService().rename(group1_1, "group1_1_1");
			AssertJUnit.fail();
		} catch (ObjectAlreadyExistsException e) {
		...
		try {
			groupService().rename(group1_1, null);
			AssertJUnit.fail();
		} catch (EJBException e) {
			assert e.getCausedByException() instanceof IllegalArgumentException;
		}

		groupService().rename(group1_1, "group1-1");
		AssertJUnit.assertEquals("group1-1", groupService().getName(group1_1));
	...
	public void setDisplayNameTest() throws Exception {
		try {
			groupService().setDisplayName(group1_1, null);
			AssertJUnit.fail();
		} catch (EJBException e) {
			assert e.getCausedByException() instanceof IllegalArgumentException;
		}
		...
		groupService().setDisplayName(group1_1, "name");
		groupService().setDisplayName(group1_2, "name");
		AssertJUnit.assertEquals("name", groupService().getDisplayName(group1_1));
		AssertJUnit.assertEquals("name", groupService().getDisplayName(group1_2));
	}
	...
	public void getGroupsByDisplayNameTest() throws Exception {
		WasabiGroupDTO group1 = groupService().getGroupByName("group1");
		groupService().setDisplayName(group1, "group1_1");
