...
public class WasabiLinkACL {
	...
	public static void ACLEntryForCreate(Node linkNode) throws UnexpectedInternalProblemException,
			ConcurrentModificationException {
		if (ACLServiceImpl.getInheritance(linkNode))
			ACLServiceImpl.setInheritance(linkNode, true);
	}

	public static void ACLEntryForMove(Node linkNode, Node newEnvironmentNode)
			throws UnexpectedInternalProblemException, ConcurrentModificationException {
		try {
			String[] inheritance_ids = WasabiLinkSQL.SQLQueryForMove(linkNode.getIdentifier(), newEnvironmentNode
					.getIdentifier());
	...
	public static void remove(Node linkNode, String callerPrincipal, Session s, boolean doJcrSave, boolean throwEvents,
			JmsConnector jms) throws UnexpectedInternalProblemException, ConcurrentModificationException {
		WasabiObjectACL.remove(linkNode, callerPrincipal, s, doJcrSave, throwEvents, jms);
	}
}
